  ...
  <div class="col">
    @foreach($usertrips as $usertrip)
      ...
      <div class="trip-highlight-card">
        <div class="card-head">
          <img src="{{$usertrip->tripCountry->countryFiles[0]->getFile()}}" alt="" />
{{--          <div class="map" id="map-1" style="height:200px;"></div>--}}
      ...
      <div class="media-body">
        <h5 class="mt-0">
          <p>{{$usertrip->title}}</p>
          <a href="{{route('user.my-trip-delete',$usertrip->id)}}" class="btn btn-light">Delete</a>
        </h5>
        <p>
          {{$usertrip->description}}
        </p>
        <div class="media-btn">
          <a href="{{route('user.my-trip-details',$usertrip->id)}}" class="btn btn-primary mr-3">
            View Trip
          </a>
