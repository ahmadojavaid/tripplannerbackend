              ...
              <div class="section-content mb-4">
                <div class="container">
                  <div class="row">
                    ...
                    @php
                    $ids = $usertrip->id.','.$usertrip->tripPlaces[0]->place_id;
                    @endphp
                    <div class="col-lg-12">
                      <button class="btn-primary btn update-tripx" value="{{$ids}}" disabled>
                        Update Trip
                      </button>
            ...
            <div class="container">
              <div class="section-head mb-3">
                <h2 class="section-title mb-1">
                  Select a place to edit your trip
...
<script>
  let data =  {
    ...
    updatetripplace:"{{route('user.my-trips-place-update')}}"
  };
  var userTrip = {!! json_encode($usertrip) !!};
  var placelocation = {!! json_encode($location) !!}
  ...
  $(document).ready(function(){
    ...
    initProcess(data,placelocation);
  });
  $(function() {
  ...
  $('body').on('click','.update-tripx',function(){
    let ids = $(this).val();
    $(".panel-default").each(function(){
      var nights = parseInt($(this).find('.no_of_nights') .val())
      var place_id = $(this).find('.handle-transport') .attr('data-id')
  ...
  function updateTripPlace(id){
    $.ajax({
      type: "POST",
      url: state.links.updatetripplace,
      data:{
        ids:id,
      },
      success: function(response) {
        if(response.status){
          window.location.href = "{{ route('user.my-trips')}}"
        }
      }
    });
  }
